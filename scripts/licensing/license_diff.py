...
from scripts.licensing.attributions_lockfile_md import LicenseInventoryEntry
from scripts.licensing.generate import collect
...
def generate_inventory(root: Path, languages: list[Language], *, label: str) -> Inventory:
    """Collect all independently locked projects, including pinned remote workspaces."""
    _status(f"{label}: collecting repository and plugin inventories")
    _, inventory = collect(root, inventory_only=True)
    return {language: inventory.get(language, []) for language in languages}
                ...
                updated_changed.append(
                    {
                        ...
                        "after": after,
                        "removed": [
                            row for row in before if _entry_identity(row) not in after_keys
                        ],
...
def _filter_inventory(inventory: Inventory, languages: list[str]) -> Inventory:
    ...
    return {
        language: inventory.get(language, []) for language in languages if language in inventory
    }
