...
CONFIG = Path(__file__).with_name("about.toml")
OUTPUT = None
MARKDOWN_CODE_FENCE = "```\n"
...
RUST_HEADER = (
    ...
    + "Regenerate with `uv run python scripts/licensing/attributions_lockfile_md.py rust --root <project-directory>`.\n\n"
)
...
Please do not edit it directly. Regenerate with `uv run python scripts/licensing/attributions_lockfile_md.py python --root <project-directory>`
"""
...
def _render_rust_metadata_fallback_attribution(crate: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if not license_files:
        license_files = _rust_upstream_licenses(crate)
    if license_files:
...
def _rust_missing_cargo_about_packages(rendered_keys: set[tuple[str, str]]) -> list[dict[str, Any]]:
    ...
    for key in sorted(lock_keys - rendered_keys, key=lambda item: (item[0].lower(), item[1])):
        ...
        if pkg is None:
            raise RuntimeError(
                f"Cargo.lock package {key[0]} {key[1]} is missing from cargo metadata"
            )
        missing.append(pkg)
    return missing
...
def _rendered_python_package_inventory(pkg: RenderedPythonPackage) -> LicenseInventoryEntry:
    ...
    return _license_inventory_entry(
        pkg["name"], pkg["version"], _normalize_license_name(pkg["license_name"])
    )


def _rust_license_inventory(
    data: dict[str, Any], workspace_members: set[str]
    ...
    return [
        ...
        for (name, version), licenses in sorted(
            rows.items(), key=lambda item: (item[0][0].lower(), item[0][1])
        )
    ]
...
def _wheel_metadata_from_bytes(
    data: bytes, *, package_name: str
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        ...
        license_texts = _declared_wheel_license_texts(
            zf, dist_info_dir, metadata.get_all("License-File", [])
        )
...
def _heuristic_sdist_license_texts(tf: tarfile.TarFile, root_dir: str) -> list[tuple[str, str]]:
    ...
    heuristic_paths = _common_license_candidates(
        [name for name in tf.getnames() if name.startswith(f"{root_dir}/")]
    )
    heuristic_candidates: list[tuple[str, str, str]] = []
    for candidate in heuristic_paths:
        text = _extract_tar_text(tf, candidate)
...
def _sdist_metadata_from_bytes(
    data: bytes, *, package_name: str
) -> tuple[str, list[tuple[str, str]]]:
    ...
    with tarfile.open(fileobj=io.BytesIO(data), mode="r:*") as tf:
        ...
        license_name = _metadata_license_name(metadata)
        license_texts = _declared_sdist_license_texts(
            tf, root_dir, metadata.get_all("License-File", [])
        )
        if not license_texts:
...
def _sdist_artifact_metadata(
    artifact: Any, package_name: str
...
def _upstream_licenses(repository: str, sha: str, subdir: str = "") -> list[tuple[str, str]]:
    """Fetch license text only from an exact GitHub source revision, never a default branch."""
    match = re.fullmatch(r"https://github.com/([\w.-]+/[\w.-]+?)(?:\.git)?/?", repository)
    if not match or not re.fullmatch(r"[0-9a-f]{40}", sha):
        raise ValueError(f"Cannot identify immutable upstream license source: {repository} {sha}")
    repo = match[1]
    prefixes = [subdir.strip("/") + "/", ""] if subdir else [""]
    for prefix in prefixes:
        ...
        for filename in [
            ...
            try:
                ...
                raise
            if text.strip():
                texts.append((f"https://github.com/{repo}/blob/{sha}/{path}", text))
        if texts and any("LICENSE" in label for label, _ in texts):
            return texts
    return []


def _rust_upstream_licenses(crate: dict[str, Any]) -> list[tuple[str, str]]:
    """Cargo archives record the exact publication commit in .cargo_vcs_info.json."""
    vcs_file = Path(crate["manifest_path"]).parent / ".cargo_vcs_info.json"
    if not vcs_file.exists():
        return []
    vcs = json.loads(vcs_file.read_text(encoding="utf-8"))
    return _upstream_licenses(
        str(crate.get("repository", "")), vcs["git"]["sha1"], vcs.get("path_in_vcs", "")
    )


def _python_upstream_licenses(pkg: dict[str, Any]) -> list[tuple[str, str]]:
    """Resolve an exact version tag from repository metadata in a hash-verified wheel."""
...
def _lockfile_only_python_package(pkg: dict[str, Any]) -> RenderedPythonPackage:
    ...
    if not license_texts:
        raise ValueError(f"Missing upstream license text for {pkg['name']} {pkg['version']}")
...
def cmd_rust() -> int:
    ...
    out = OUTPUT or ROOT / "ATTRIBUTIONS-Rust.md"
...
def cmd_python() -> int:
    ...
    out = OUTPUT or ROOT / "ATTRIBUTIONS-Python.md"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate lockfile-scoped notices in NeMo Relay's format."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT,
        help="Project or pinned workspace containing the lockfile",
    )
    parser.add_argument(
        "--output", type=Path, help="Output path (defaults to the project directory)"
    )
    args = parser.parse_args()
    ROOT = args.root.resolve()
    OUTPUT = args.output.resolve() if args.output else None
    return cmd_python() if args.language == "python" else cmd_rust()
